...
class WebsiteScraper:
    ...
    def _is_actor1_roadblock(self, website_map: dict) -> bool:
        """🚧 ENHANCED: Detect if Actor 1 hit a roadblock (JavaScript, empty content, invalid URLs, etc.)"""
        ...
        # 🔧 CRITICAL: Check for valid internal links
        valid_internal_links = 0
        for link in all_links:
            href = link.get('href', '')
            # Count only valid navigable URLs
            if (href.startswith('/') or href.startswith('http')) and not any(invalid in href.lower() for invalid in ['mailto:', 'tel:', 'javascript:']):
                valid_internal_links += 1
        
        roadblock_detected = (
            total_links < 3 or              # Very few links found
            valid_internal_links < 2 or     # Very few valid internal links
        ...
        if roadblock_detected:
            ...
            print(f"   📊 Total links: {total_links} (need ≥3)")
            print(f"   ✅ Valid internal links: {valid_internal_links} (need ≥2)")
            print(f"   📄 Has content: {has_content}")
            print(f"   📝 Content length: {body_length} chars (need ≥200)")
    ...
    def _gpt4o_direct_web_access_workflow(self, website_url: str) -> tuple:
        """🤖 FIXED: GPT-4o with web search capabilities - finds actual staff pages dynamically"""
        
        print("🤖 GPT-4O WEB SEARCH + ANALYSIS WORKFLOW")
        print("🔧 Using GPT-4o web search to find and analyze actual staff pages")
        print("=" * 70)
        ...
        try:
            ...
            # Phase 1: GPT-4o web search for staff pages
            print("🔍 Phase 1: GPT-4o searching web for staff/team pages...")
            
            search_prompt = f"""You can search the web. Please search for staff/team information for {domain}.

TASK: Use web search to find staff/team pages and LinkedIn company profile for {domain}.

SEARCH QUERIES TO RUN:
1. Search: "site:{domain} team" 
2. Search: "site:{domain} staff"
3. Search: "site:{domain} about"
4. Search: "{domain} employees team members"
5. Search: "{domain} LinkedIn company"

For each search result, analyze and identify:
- URLs that likely contain staff/employee information
- The company's LinkedIn profile URL
- Any team/about pages with employee listings

Return your findings in this format:
STAFF_PAGE_URLS:
1. [full_url] - [why this page likely has staff info]
2. [full_url] - [why this page likely has staff info]
3. [full_url] - [why this page likely has staff info]

LINKEDIN_URL: [company LinkedIn URL if found]

SEARCH_ANALYSIS: [summary of what you found through web search]"""
            ...
            print(f"🧠 GPT-4o Web Search Results:")
            print(f"{search_result}")
            
            # Extract URLs from search results
            staff_urls = self._extract_urls_from_search_results(search_result, website_url)
            linkedin_url = self._extract_linkedin_from_search_results(search_result)
            
            if not staff_urls:
                print("❌ GPT-4o web search found no staff URLs")
                return [], linkedin_url or ""
            
            print(f"🎯 GPT-4o found {len(staff_urls)} potential staff URLs via web search")
            
            # Phase 2: GPT-4o analyzes each found URL for staff content
            all_staff = []
            
            for i, url in enumerate(staff_urls, 1):
                print(f"\n🔍 Phase 2.{i}: GPT-4o analyzing staff content from {url}")
                
                staff_extraction_prompt = f"""Please visit and analyze the webpage: {url}

TASK: Extract staff/employee information from this page.

EXTRACTION RULES:
1. Find ONLY current employees of {domain}
2. Extract: Full name + Job title/position
3. EXCLUDE: Clients, testimonials, external consultants
4. EXCLUDE: Company names mistaken as people  
5. Require: First name + Last name minimum

Visit the URL and extract any staff information you find.
...
Please actually visit the URL and analyze the content."""

                staff_response = client.chat.completions.create(
                    model="gpt-4o",
                    messages=[{"role": "user", "content": staff_extraction_prompt}],
                    max_tokens=2000,
                    temperature=0.1
                )
                
                staff_result = staff_response.choices[0].message.content.strip()
                print(f"   🧠 GPT-4o extraction result: {staff_result[:200]}...")
                
                # Parse the staff data
                staff_from_page = self._parse_gpt4o_staff_response(staff_result, domain)
            ...
            print(f"\n🎉 GPT-4o Web Search + Analysis Results:")
            print(f"   🔍 Method: GPT-4o web search + direct page analysis")
            ...
            print(f"❌ GPT-4o web search workflow failed: {e}")
            return [], ""
...
"""
    ...
    def _gpt_analyze_urls_enhanced(self, website_map: dict, domain: str) -> list:
        ...
        # 🔧 CRITICAL FIX: Check if we have meaningful links
        ...
        for link in all_links[:30]:
            ...
            # Only include actual navigable URLs
...
1. ONLY select FULL URLs that start with {base_url}
2. Convert relative URLs (starting with /) to full URLs
3. NEVER select mailto:, tel:, or javascript: links
4. PRIORITIZE: team, about, staff, people, leadership, contact pages
...
TASK: Select 3 BEST FULL URLs for staff extraction
