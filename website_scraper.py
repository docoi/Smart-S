"""
🌐 Enhanced Website Scraper Module - PART 0 INTEGRATION
=======================================================
Integrates with Part 0 Recon System for guaranteed data discovery and LinkedIn URL extraction

WORKFLOW:
1. Check for Part 0 cache files (from your recon system)
2. If missing, run Part 0 pipeline automatically  
3. Consume Part 0 data for enhanced analysis
4. Use GPT-4o for intelligent staff validation and URL analysis
5. Return clean staff list + guaranteed LinkedIn URL

PART 0 FILES CONSUMED:
- cache_external_urls.txt → GPT-4o URL intelligence
- site_social_links.json → LinkedIn URL for Part 2
- staff_scrape_results.json → Pre-extracted staff data
- cache_items_full.json → Rich content for analysis
"""
...
class WebsiteScraper:
    """🌐 Enhanced website scraper with Part 0 Recon integration"""
    
    def __init__(self, openai_key):
        self.openai_key = openai_key
        self.script_dir = Path(__file__).parent
        
        # Part 0 cache file paths
        self.cache_files = {
            'external_urls': self.script_dir / 'cache_external_urls.txt',
            'social_links': self.script_dir / 'site_social_links.json',
            'staff_results': self.script_dir / 'staff_scrape_results.json',
            'items_full': self.script_dir / 'cache_items_full.json',
            'urls_all': self.script_dir / 'cache_urls_all.json'
        }
    ...
    def scrape_website_for_staff_and_linkedin(self, website_url: str) -> Tuple[List[Dict], str]:
        """📋 MAIN METHOD: Enhanced website scraping with Part 0 Recon integration"""
        ...
        # PHASE 1: Ensure Part 0 data exists
        print(f"🔍 PHASE 1: PART 0 RECON DATA CHECK")
        print("-" * 40)
        
        if not self._part0_data_exists():
            print("📦 Part 0 cache files missing - running Part 0 pipeline...")
            self._run_part0_pipeline(normalized_url)
        else:
            print("✅ Part 0 cache files found - proceeding with analysis")
        
        # PHASE 2: Load Part 0 data
        print(f"\n📊 PHASE 2: LOADING PART 0 DATA")
        print("-" * 40)
        ...
        print(f"\n🧠 PHASE 3: ENHANCED ANALYSIS WITH GPT-4O")
        print("-" * 40)
    ...
    def _part0_data_exists(self) -> bool:
        """🔍 Check if Part 0 cache files exist and are recent"""
        
        required_files = ['social_links', 'external_urls']
        ...
        for file_key in required_files:
            ...
            if age_seconds > 3600:  # 1 hour
                print(f"   ⏰ Stale: {file_path.name} ({age_seconds/60:.1f} min old)")
                return False
    ...
    def _run_part0_pipeline(self, url: str) -> bool:
        """🚀 Run Part 0 pipeline to generate cache files"""
        ...
        try:
            ...
            # Run Part 0 pipeline with encoding fix
            ...
            print(f"   ⏰ Part 0 pipeline timed out after 10 minutes")
            return False
        except Exception as e:
    ...
    def _run_part0_fallback(self, url: str) -> bool:
        ...
        try:
            ...
            env = os.environ.copy()
            env['PYTHONIOENCODING'] = 'utf-8'
            env['INCLUDE_SITEMAPS'] = '0'  # Disable sitemaps for faster execution
    ...
    def _load_part0_data(self) -> Dict[str, Any]:
        ...
        try:
            with open(self.cache_files['social_links'], 'r', encoding='utf-8') as f:
                ...
                print(f"   ✅ Loaded social links: {len(data['social_links'].get('all', []))} platforms")
        except Exception as e:
...
If no valid staff: []"""
        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                return validated_staff[:15]  # Limit to 15 staff
            
        except Exception as e:
            print(f"   ⚠️ GPT validation failed: {e}")
        
        # Fallback: return original list with basic validation
        return [s for s in staff_list if self._is_valid_person_name(s.get('name', ''))]
    ...
    def _gpt_analyze_urls_for_staff(self, part0_data: Dict[str, Any], domain: str) -> List[Dict[str, str]]:
        ...
        urls_text = "\n".join([f"{i+1}. {url}" for i, url in enumerate(staff_urls[:10])])
        
        prompt = f"""Analyze these URLs from {domain} and identify the 3 most likely to contain staff information.

AVAILABLE URLS:
{urls_text}

Select the 3 BEST URLs for finding current employees/staff.
Return as JSON:
{{
  ...
  "reasoning": "why these URLs were selected"
        ...
        try:
            ...
            result_text = response.choices[0].message.content.strip()
            print(f"   🧠 GPT-4o URL analysis complete")
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                print(f"   🎯 Selected {len(selected_urls)} URLs for staff analysis")
                print(f"   💭 Reasoning: {reasoning}")
                
                # For now, return empty as we'd need to scrape these URLs
                # This could be enhanced to actually scrape the selected URLs
                return []
            
        except Exception as e:
            print(f"   ⚠️ GPT URL analysis failed: {e}")
