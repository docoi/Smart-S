"""
🌐 Website Scraper Module - FIXED LINKEDIN URL EXTRACTION
=========================================================
Fixed LinkedIn URL corruption and GPT-4o URL selection issues
"""
...
class WebsiteScraper:
    ...
    def scrape_website_for_staff_and_linkedin(self, website_url: str) -> tuple:
        ...
        # Extract staff and LinkedIn from selected URLs using APIFY_TOKEN_1
        staff_list, social_data = self._extract_staff_and_social(selected_urls)
        
        # Find LinkedIn URL with enhanced cleaning
        linkedin_url = self._get_clean_linkedin_url(social_data, website_map)
...
IMPORTANT: Focus ONLY on: about, team, staff, people, contact, leadership pages from the SAME DOMAIN."""
    ...
    def _get_clean_linkedin_url(self, social_data: dict, website_map: dict) -> str:
        """🔧 FIXED: Get clean LinkedIn URL from social data AND website map"""
        
        linkedin_candidates = []
        
        # 1. Collect from social media data
        for key, value in social_data.items():
            if isinstance(value, str) and 'linkedin.com' in value.lower():
                linkedin_candidates.append(value)
        
        # 2. Collect from website map links
        if website_map and 'allLinks' in website_map:
            for link in website_map['allLinks']:
                url = link.get('url', '')
                if 'linkedin.com' in url.lower():
                    linkedin_candidates.append(url)
        
        if not linkedin_candidates:
            print("⚠️ No LinkedIn URLs found in website content")
            return ""
        ...
        for candidate in linkedin_candidates:
            ...
            if cleaned_url:
                print(f"🔗 Cleaned LinkedIn URL: {cleaned_url}")
                return cleaned_url
        
        print("⚠️ No clean LinkedIn URL could be extracted")
        return ""

    def _clean_linkedin_url(self, raw_url: str) -> str:
        ...
        try:
            ...
            if len(path_parts) >= 2 and path_parts[0] == 'company':
                ...
                if 'company' in parsed.path:
                    ...
                    # Assume first path part is company name
            ...
            # Validate the company name looks reasonable
            if len(company_name) < 2 or len(company_name) > 100:
                print(f"    ⚠️ Invalid company name length: {company_name}")
                return ""
