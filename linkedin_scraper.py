"""
🔗 LinkedIn Scraper Module - UPDATED FOR FULL EMAIL SEARCH
==========================================================
Uses APIFY_TOKEN_1 with Full + email search mode ($12 per 1000)
"""
...
class LinkedInScraper:
    """🔗 LinkedIn scraping with Full + email search mode"""
    ...
    def _full_email_search_actor2(self, linkedin_url: str, domain: str) -> list:
        """🔍 Full + email search Actor 2 with APIFY_TOKEN_1"""
        ...
        try:
            ...
            for item in items:
                try:
                    # Extract employee data with enhanced email extraction
                    name = f"{item.get('firstName', '')} {item.get('lastName', '')}".strip()
                    title = (item.get('headline', '') or 
                           item.get('position', '') or 
                           item.get('currentPosition', [{}])[0].get('title', '') if item.get('currentPosition') else '' or
                           'Employee')
