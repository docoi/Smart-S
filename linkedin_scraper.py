...
class LinkedInScraper:
    """🔗 LinkedIn scraping with smart pattern learning and enhanced fire protection targeting"""
    ...
    def scrape_linkedin_and_discover_emails(self, linkedin_url: str, domain: str) -> list:
        ...
        try:
            ...
            # Step 2: ENHANCED Fire protection targeting - ONLY SENIOR DECISION MAKERS
    ...
    def _score_fire_protection_targets_enhanced(self, linkedin_contacts: list, max_targets: int = 2) -> list:
        """🎯 ENHANCED: Fire protection targeting - ONLY SENIOR DECISION MAKERS"""
        
        print("🔥 ENHANCED FIRE PROTECTION CONTACT IDENTIFICATION")
        print(f"🎯 Target: {max_targets} SENIOR decision-makers with fire protection purchasing authority")
        print("🚨 REQUIREMENT: Only directors, managers, and business owners qualify")
        print("=" * 70)
        ...
        # ENHANCED Fire protection scoring - ONLY SENIOR DECISION MAKERS
        scoring_criteria = {
            'owner_director': {
            ...
            'facilities_head': {
                'keywords': ['facilities director', 'facilities manager', 'head of facilities', 'building manager'],
                'score': 95,
                'reason': 'Facilities leadership - direct responsibility for building safety systems and fire protection'
            },
            'operations_director': {
                'keywords': ['operations director', 'head of operations', 'operations manager'],
                'score': 90,
                'reason': 'Operations leadership - oversees safety procedures and has budget authority'
            },
            'safety_head': {
                'keywords': ['safety director', 'head of safety', 'safety manager', 'hse manager', 'health and safety manager'],
                'score': 95,
                'reason': 'Safety leadership - direct fire protection responsibility with decision-making authority'
            },
            'general_manager': {
                'keywords': ['general manager', 'site manager', 'branch manager'],
                'score': 85,
                'reason': 'General management - site responsibility and budget authority for safety investments'
            }
        }
        ...
        for contact in contacts_with_emails:
            ...
            best_score = 0
            best_reason = 'Not a senior decision maker for fire protection services'
            
            # Score against each criteria - REQUIRE EXACT MATCHES
            ...
            # ONLY include contacts with scores above 80 (senior decision makers only)
            if best_score >= 80:
                contact['fire_protection_score'] = best_score
                contact['fire_protection_reason'] = best_reason
                qualified_contacts.append(contact)
                
                print(f"   ✅ QUALIFIED: {contact['name']} - {contact['title']} | Score: {best_score} | {contact['email']} | {best_reason}")
            else:
                print(f"   ❌ NOT QUALIFIED: {contact['name']} - {contact['title']} | Score: {best_score} | Not senior enough for fire protection purchasing decisions")
        
        if not qualified_contacts:
            print("⚠️ NO QUALIFIED SENIOR DECISION MAKERS FOUND")
            print("🔍 Consider expanding search or checking for business owners/directors")
            return []
    ...
    def _native_scrape_linkedin_actor2(self, linkedin_url: str, domain: str) -> list:
        ...
        try:
            ...
            client = manager.get_working_apify_client_part2()
